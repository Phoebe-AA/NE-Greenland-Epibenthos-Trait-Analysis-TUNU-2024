---
title: "NE Greenland Trait Analysis: HMSC"
...
XFormula is for the environmental input you want for the model 
```{r}
...
The taxa data is zero-inflated and comprised of many rare occurrences. Additionally, there are few stations within the habitats and no replicates. Therefore, a more conservative model will be used. Since the abundance dataset has recorded more years than the biomass, then the abundance dataset will be used. Henceforth, here is the code to convert the presence-absence model: 
```{r}
Ypa <- 1*(Y>0)
```
...
Reassign into syntax (NB: This is a good way for when running multiple models)
```{r}
models <- list(m1)
