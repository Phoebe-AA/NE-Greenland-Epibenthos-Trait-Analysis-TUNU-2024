...
library(tidyr)
```
#### Load model and set new parameters
include in samples_list and thin_list only those model that you have actually fitted
```{r}
...
#### Evaluate converge 
##### Checking BETA parameter diagnostics 
```{r}
...
model
```
##### Checking GAMMA parameter diagnostics
```{r}
...
##### Checking RHO parameter diagnostics
```{r}
...
```{r}
subset(model$XData, model$XData$Habitat=="Fjord")
subset(model$XData, model$XData$Habitat=="Shelf")
subset(model$XData, model$XData$Habitat=="Shelfbreak")
subset(model$XData, model$XData$Habitat=="Slope")
```

```{r}
plot(Depth~Station, col=Habitat, pch=19,data = model$XData)
plot(Oxygen~Station, col=Habitat, pch=19,data = model$XData)
plot(Flourescence~Station, col=Habitat, pch=19,data = model$XData)
plot(B.Temp~Station, col=Habitat, pch=19,data = model$XData)
plot(B.PSU~Station, col=Habitat, pch=19,data = model$XData)
plot(Turbidity~Station, col=Habitat, pch=19,data = model$XData)
```

```{r}
par(mfrow=c(3,2), mar=c(4,4.5,1,0.5))
boxplot(Depth~Habitat, pch=19,data = model$XData,
        ylab = "Water Depth (m)")
boxplot(B.Temp~Habitat, pch=19,data = model$XData,
        ylab = "Bottom Temperature (\u00B0C)")
boxplot(B.PSU~Habitat, pch=19,data = model$XData,
        ylab = "Bottom Salinity (PSU)")
boxplot(Oxygen~Habitat, pch=19,data = model$XData,
        ylab = expression(paste("Bottom Oxygen (ml L"^" -1",")")))
boxplot(Turbidity~Habitat, pch=19,data = model$XData,
        ylab = "Turbidity (FTU)")
boxplot(Flourescence~Habitat, pch=19,data = model$XData,
        ylab = "Fluorescence")
```
